...
typedef struct _neuralnet
{
	...
	/* variational autoencoder latent space vectors */
	t_float *x_mean;
	t_float *x_sigma;
	int x_vae_latent_output_size;
	int x_vae_latent_space_allocated;
	...
	int x_pred_to_added;
	/* detect if we create a variational autoencoder */
	int x_is_vae;
	int x_vae_latent_ndx;
	t_float x_vae_kl;
	/* detect whether we create an encoder or decoder of an autoencoder */
	int x_is_encoder;
	int x_is_decoder;
	/* adding WAV files as training dataset */
...
static void add_audio_file(t_neuralnet *x, t_symbol *s, const char *audio_file);
static void add_files(t_neuralnet *x, t_symbol *s);
